...
# -*- coding: utf-8 -*-
"""
使用case7.json标准格式测试医疗诊断Agent系统

这个脚本读取case7.json文件，将其转换为系统所需的格式，
...
def test_case7():
    ...
    try:
        ...
        # 读取case7数据
        with open('medical_records/case7.json', 'r', encoding='utf-8') as f:
            case_data = json.load(f)
        
        print('\n' + '='*80)
        print('🏥 开始测试 case7.json')
        print('='*80)
        ...
        try:
            ...
            report_filename = f"output/case7_diagnosis_report_{timestamp}.json"
            
            # 导出报告
            report_path = orchestrator.export_diagnosis_report(result, 'json')
            
            # 重命名为case7专用文件名（如导出未落盘则手动保存）
            import os

...
# -*- coding: utf-8 -*-
"""
使用case5.json标准格式测试医疗诊断Agent系统

这个脚本读取case5.json文件，将其转换为系统所需的格式，
...
def test_case5():
    ...
    try:
        ...
        # 读取case5数据
        with open('medical_records/case5.json', 'r', encoding='utf-8') as f:
            case_data = json.load(f)
        
        print('\n' + '='*80)
        print('🏥 开始测试 case5.json')
        print('='*80)
        ...
        try:
            ...
            report_filename = f"output/case5_diagnosis_report_{timestamp}.json"
            
            # 导出报告
            report_path = orchestrator.export_diagnosis_report(result, 'json')
            
            # 重命名为case5专用文件名（如导出未落盘则手动保存）
            import os
